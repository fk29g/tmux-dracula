...
main() {
  ...
  if $show_powerline; then
    if $show_edge_icons; then
      ...
      tmux set-option -g status-left "#[bg=${dark_gray},fg=${green}]#[bg=${light_purple},fg=${dark_gray}]#{?client_prefix,#[bg=${yellow}],} ${left_icon} #[fg=${light_purple},bg=${bg_color}]#{?client_prefix,#[fg=${yellow}],}${left_sep}"
    fi
    powerbg=${bg_color}
  else
  ...
  # Window option
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[fg=${window_sep_fg},bg=${window_sep_bg}]${window_sep}#[fg=${dark_gray},bg=${dark_purple}] #I #W${current_flags} #[fg=${dark_purple},bg=${bg_color}]${left_sep}"
  else
